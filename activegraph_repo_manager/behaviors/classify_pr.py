"""Deterministic PR classification replay using recorded fixture output."""
...
def classification_input_hash(pr_payload: dict[str, Any], changed_files: list[str]) -> str:
    metadata = pr_payload.get("external_metadata", {})
    canonical = {
        "title": pr_payload.get("title", ""),
        "body": metadata.get("body", ""),
        "labels": sorted(metadata.get("labels", [])),
        "state": pr_payload.get("status", ""),
...
def classify_pr_from_recorded_output(
    pr_payload: dict[str, Any], changed_files: list[str], recorded_output: dict[str, Any]
) -> dict[str, Any]:
    payload = dict(recorded_output)
    payload["classification_input_hash"] = classification_input_hash(pr_payload, changed_files)
